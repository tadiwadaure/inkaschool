...
try {
    ...
    // Get classes for this teacher (based on assigned subjects)
    ...
    if (!empty($classIds)) {
        ...
        $classesStmt = $pdo->prepare("
            ...
            ORDER BY class_name
        ");
        $classesStmt->execute($classIds);
        $teacherClasses = $classesStmt->fetchAll();
    }
    
} catch (PDOException $e) {
    error_log('Teacher exams error: ' . $e->getMessage());
    $assignedSubjects = [];
    $teacherClasses = [];
...
try {
    if (!empty($classIds)) {
        ...
        $examsStmt = $pdo->prepare("
            ...
            WHERE e.class_id IS NULL OR e.class_id IN ($placeholders)
    ...
    $exams = [];
    $message = 'Error loading exams: ' . $e->getMessage();
    $messageType = 'danger';
}
...
<div class="row">
    <div class="col-12">
        <h1 class="h3 mb-4">
            ...
            <small class="text-muted">Create and manage examinations for your classes</small>
        </h1>
    </div>
</div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="exam_name" name="exam_name" 
                                placeholder="e.g., Unit Test, Final Examination" required>
                        </div>
                        <div class="col-md-6 mb-3">
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">
                    <i class="fas fa-list"></i> Available Exams
                </h5>
            </div>
            <div class="card-body">
                <?php if (!empty($exams)): ?>
